...
async def parse_preview(file: UploadFile = File(...)):
    try:
        ...
        try:
            # Wczytaj cały plik, aby znaleźć wszystkie braki danych
            df_full = pd.read_csv(io.BytesIO(content), encoding='utf-8')
        except UnicodeDecodeError:
            # Spróbuj innego kodowania, jeśli domyślne zawiedzie
            df_full = pd.read_csv(io.BytesIO(content), encoding='latin1')

        # Analiza brakujących wartości w całym pliku
        ...
        # Generowanie podglądu z pierwszych 5 wierszy
        ...
        # Zwróć szczegółowy błąd, jeśli przetwarzanie pliku się nie powiedzie
