...
async def parse_preview(file: UploadFile = File(...)):
    try:
        ...
        has_missing_values = df_preview.isnull().values.any()
        ...
        return JSONResponse(status_code=400, content={"error": f"Błąd przetwarzania pliku CSV: {e}"})
...
def run_academic_tests_and_build_table(df: pd.DataFrame, variable_types: dict, missing_data_info: str) -> str:
    ...
    for col1, col2 in itertools.combinations(categorical_cols, 2):
        ...
        try:
            ...
            print(f"--- DEBUG START ---")
            print(f"--- VARS: {col1} vs {col2} ---")
            print(f"--- chi2_result TYPE: {type(chi2_result)} ---")
            if isinstance(chi2_result, (list, tuple)):
                print(f"--- chi2_result LEN: {len(chi2_result)} ---")
                if len(chi2_result) > 0:
                    print(f"--- stats_df (element 0) TYPE: {type(chi2_result[0])} ---")
                    print(f"--- stats_df (element 0) CONTENT: ---")
                    print(chi2_result[0])
            print(f"--- DEBUG END ---")
            stats_df, expected, _ = chi2_result
            p_value_chi2 = stats_df.loc[stats_df['test'] == 'pearson', 'p-val'].iloc[0]
...
async def generate_report(
    ...
    profile = ProfileReport(df, title="Część 1: Automatyczny Raport Opisowy (Podstawowy)", minimal=True, correlations=None, interactions=None, missing_diagrams=None)
